...
using tkpl.Model;
    ...
    private static void InitializeFallbackData()
    {
        ...
        {
            var fallbackModule = new ReportQuizItem(1, "Soal tidak ditemukan", "N/A" , 0, false);
    ...
    public static async Task FetchLevelsFromApiAsync()
    {
        try
        {
            ...
            if (reportQuizItemsFromApi != null)
            {
                ...
                {
                    var newReportItem = new ReportQuizItem(apiRep.id, apiRep.QuestionText, apiRep.CorrectAnswer , apiRep.QuestionIndex, apiRep.IsCorrect);
...
public class ReportQuizItem
{
    ...
    public ReportQuizItem(int id, string questionText, string correctAnswer, int questionIndex, bool isUserCorrect)
    {
        this.id = id;
