...
public class LogicLevel
{
    ...
    public LogicLevel()
    {
        if (RepoLevel.MasterTable.Length > 0)
        {
            ResetLives();
        }
    }
    ...
    public void ProcessAnswer(string input)
    {
        ...
        // Logika Validasi Jawaban (Invariant)
    ...
    {
        // Logika Perpindahan Level (Post-Condition)
        if (_currentLessIdx < RepoLevel.MasterTable[_currentModIdx].Lessons.Length - 1)
        {
            _currentLessIdx++;
        }
        else if (_currentModIdx < RepoLevel.MasterTable.Length - 1)
        {
            _currentModIdx++;
            _currentLessIdx = 0;
            ResetLives();
